...
export function parseTournamentsProgrammatically(text: string): Tournament[] {
  ...
  const categoryPatterns = [
    ...
    { label: "11U", regex: /11\s*&\s*U\s*EVENTS/i },
    { label: "12U", regex: /12\s*&\s*U\s*EVENTS/i },
    { label: "14U", regex: /14\s*&\s*U\s*EVENTS/i },
    { label: "16U", regex: /16\s*&\s*U\s*EVENTS/i },
    { label: "18U", regex: /18\s*&\s*U\s*EVENTS/i },
    { label: "Open Men", regex: /Open\s*Men/i },
    { label: "Open Women", regex: /Open\s*Women/i },
  ];
  ...
  for (let i = 0; i < codeMatches.length; i++) {
    ...
    // We remove the code and common separators
    ...
    title = title.replace(/South\s*&\s*South\s*West\s*Tour\s*-\s*/gi, '');
